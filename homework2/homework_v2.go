...
func main() {
	...
	const usersCount = 100 // amount of workers should equal amount of users
	users := make(chan User, usersCount)

	for i := 0; i < usersCount; i++ {
		wg.Add(1)
		go saveUserInfoWorker(i+1, users, wg)
	}

	for i := 0; i < usersCount; i++ {
		go generateUser(i+1, users)
	}
...
func saveUserInfoWorker(id int, users <-chan User, wg *sync.WaitGroup) {
